...
class FakeAVCelebDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> List[Tensor]:
        ...
        file_path = os.path.join(self.root, meta.vid, meta.path)
        ...
        return {"id": index, "file": os.path.join(meta.vid, meta.path), "video": video, 'audio': audio, "v_label": torch.tensor(v_label),
...
class FakeavcelebDataModule:
    """
    Standard PyTorch DataModule (not using Lightning)
    Handles train/test dataset creation and DataLoader management
    """
    train_dataset: FakeAVCelebDataset
    dev_dataset: FakeAVCelebDataset
    test_dataset: FakeAVCelebDataset
    ...
    def __init__(self, root: str = "data",
        ...
        self.Dataset = FakeAVCelebDataset
        self.pad_audio = False
    ...
    def val_dataloader(self) -> DataLoader:
        """Return validation DataLoader (same as test)"""
    ...
    def get_datasets(self):
        """Return train and test datasets directly"""
        return self.train_dataset, self.test_dataset

    def get_dataloaders(self):
        """Return train and test dataloaders directly"""
        return self.train_dataloader(), self.test_dataloader()


# Utility functions for direct dataset/dataloader creation
def create_fakeavceleb_datasets(root: str, train_fold: str):
    """
    Create FakeAVCeleb datasets directly without DataModule

    Args:
        root: Root directory path
        train_fold: Training fold file name

    Returns:
        train_dataset, test_dataset
    """
    dm = FakeavcelebDataModule(root=root, train_fold=train_fold)
    return dm.get_datasets()


def create_fakeavceleb_dataloaders(root: str, train_fold: str, batch_size: int = 32,
                                   num_workers: int = 4, type: str = 'train'):
    """
    Create FakeAVCeleb dataloaders directly without DataModule
    ...
    Args:
        ...
        type: 'train' returns (train_loader, val_loader), 'test' returns test_loader

    Returns:
        Depending on type:
        - 'train': (train_dataloader, val_dataloader)
        - 'test': test_dataloader
        - other: (train_dataloader, val_dataloader, test_dataloader)
    """
    ...
    if type == 'train':
        ...
        # If type is not specified, return all dataloaders
